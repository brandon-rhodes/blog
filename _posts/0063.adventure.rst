...
Here, for example, is an except from ``advent.for``
...
and therefore is written “forwards” even in the FORTRAN code.)
...
De Morgan's laws state, basically,
...
you can swap the expression's ``and`` and ``or`` operators
and the result will be an expression that means precisely the opposite.
...
was taking FORTRAN code that said one thing
and making it say more or less the opposite,
